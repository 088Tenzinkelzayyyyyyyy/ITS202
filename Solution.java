...
public class Solution<Key extends Comparable<Key>, Value>{
    ...
    //return the number of key-value paris in the tree
    public int size(){
        return size(root);
    }
    ...
    public void put(Key key, Value val){
        ...
        //check if the key exist and replace the value and stop the size from incrementing
        if(value_replace(root, key, val)) return;

        Node node = null, x = root,z = new Node(key, val, 1);
        //traverse through tree and compare key
        while (x != null) {
            node = x;
        ...
        //incrementing the size of present node
   ...
   //return the associate value of key if the key exist else return null
    ...
    //traverse through the tree and return Node with smallest key
    ...
    //returns the greatest key from the least keys in the tree which is less than or equal to given key.
    ...
    public static void main(String[] args){ 
        ...
        Solution<String, Integer> outer = new Solution<String, Integer>();
